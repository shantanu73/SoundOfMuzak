# Code for Iterative deepening Depth first search for liquid sorting game
import copy
import queue
import time
import pdb
            ...
            if i != j:
                ...
                if val:
                    if len(answer) < 2 or (len(answer) > 1 and answer[len(answer)-2] != (i, j)):
                        answer.append((i, j))
                        if id_dfs_func(state_temp, level-1):
                            return True
